...
class FSShell():
  ...
  def show_request(self):
    print(" ")
    put_requests = self.FileObject.RawBlocks.get_put_request()
    get_requests = self.FileObject.RawBlocks.get_get_request()
    print("Put() request(s): " + str(put_requests))
    print("Get() request(s): " + str(get_requests))
    total_put = 0
    total_get = 0
    for i in range(self.FileObject.RawBlocks.N):
      total_put += put_requests[i]
      total_get += get_requests[i]
    print("Average Put() request(s): " + str(total_put / N))
    print("Average Get() request(s): " + str(total_get / N))
